...
### 2. 비전 검사 실행
- 시작/정지 버튼으로 카메라 직접 제어
- 백그라운드 스레드 + Dispatcher.Invoke 로 30fps 실시간 영상 출력
- 좌우 반전(FlipMode.Y) 으로 거울 모드 적용
- HSV 변환 후 빨간색 두 구간(H: 0~10, 170~180) 마스킹으로 정확한 색상 감지
- 빨간색 비율 5% 기준으로 자동 OK/NG 판별 후 영상 위에 결과 텍스트 오버레이
- 작업지시 선택 시 1초 간격으로 검사 결과 자동 DB 저장
- OK/NG 카운트 실시간 표시
- 수동 OK/NG 버튼으로 직접 결과 입력 가능
